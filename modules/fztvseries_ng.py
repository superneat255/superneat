...
class FztvseriesNg():
    ...
    async def get_episode_dld_url(self, url):
        ...
        while int(time.time()) <= timeout:
            try:
                ...
                if r.status_code == 200:
                    dld_url = f"{parsed_url.scheme}://{parsed_url.netloc}{r.json()['file']}"
                    break
            except: pass
    ...
    async def download_here(self, episode):
        dld_url = await self.get_episode_dld_url(episode.get('dld_page_url'))
...
if __name__ == '__main__':
    async def main():
        ...
        await f.download_here(episodes[0])
